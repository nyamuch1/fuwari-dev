...
description: '这个指南解释如何在 Ubuntu 上如何搭建我的世界服务器。我们将会使用 Systemd 来运行我的世界服务器以及 mcrcon 工具来连接运行的实例。我们也将向你展示如何创建一个计划任务，执行常规的服务器备份。'
image: './image/01.png'
tags: ['mc','游戏服务器','ubuntu']
category: '指南'
draft: false 
---
...
在这个指南中，我们将会安装最新的 [Fabric](https://fabricmc.net/use/server/) 我的世界服务器。同样的指令，同样适合于其他的服务器 mods。
...
第一次启动的时候，服务器会执行一些操作，最后创建 `server.properties`和 `eula.txt`文件，并且停止。
...
```bash
sudo vim /etc/systemd/system/mcserver.service
```
